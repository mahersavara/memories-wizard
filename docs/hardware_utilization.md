...
| :--- | :--- | :--- |
| Auto-play, muted, looping video previews | Native playback stacks (`MediaElement`, `AVPlayer`, SwiftUI `VideoPlayer`) still support the current preview behavior. | Staying with an OS-native playback approach reduces control over frame-accurate behavior and makes a future zero-latency, multi-engine playback design less automatic. |
| Tinder-style swipe flow | Animation built around translate, rotate, and opacity transforms keeps swipe motion, tilt, fades, and card transitions responsive. | It intentionally deprioritizes heavier visual effects, so future glass-heavy or shader-heavy transitions may need extra profiling before adoption. |
...
| Retry-based file-lock handling during move/trash | `BitmapCacheOption.OnLoad` and a limited number of active media items reduce how long files stay tied to preview components. | Using `OnLoad` increases bitmap memory use, and the single-preview-player approach can still leave a short playback initialization delay before the next video is ready. |
| Jump to Index and fast item changes | Keeping memory bounded helps navigation stay predictable even in large folders. | The guideline avoids broad preloading, so jumping to a later large video file may still incur initial loading time instead of instant playback. |
